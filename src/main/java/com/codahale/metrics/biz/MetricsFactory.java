...
public class MetricsFactory implements InitializingBean, DisposableBean {
	...
	private <T> T getMetric(Class<T> metricClass, Class<?> clazz, String... names) {
		String key = MetricRegistry.name(metricClass.getName() + "." + clazz.getName() , names);
		Metric ret = COMPLIED_METRICS.get(key);
		if (ret != null) {
			return (T) ret;
		}
		if (metricClass == Histogram.class) {
			ret = this.getRegistry().histogram(MetricRegistry.name(clazz, names));
		}
		if (metricClass == Timer.class) {
			ret = this.getRegistry().timer(MetricRegistry.name(clazz, names));
		}
		if (metricClass == Meter.class) {
			ret = this.getRegistry().meter(MetricRegistry.name(clazz, names));
		}
		if (metricClass == Counter.class) {
			ret = this.getRegistry().counter(MetricRegistry.name(clazz, names));
		}
